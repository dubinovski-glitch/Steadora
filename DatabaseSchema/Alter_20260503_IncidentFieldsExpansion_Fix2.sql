/* ----------------------------------------------------------------------------
   Alter_20260503_IncidentFieldsExpansion_Fix2.sql
   Applies only what is still missing after the two previous failed runs:
   - RelatedKbArticleId is now BIGINT (fixed by Fix.sql)
   - FK_Inc_Caller through FK_Inc_RelatedChange already exist
   - Only FK_Inc_RelatedKbArticle and the stored procedure are still missing
   Date: 2026-05-03
   ---------------------------------------------------------------------------- */

ALTER TABLE itil.Incident ADD CONSTRAINT FK_Inc_RelatedKbArticle
    FOREIGN KEY (RelatedKbArticleId) REFERENCES kb.Article(ArticleId);
GO
